...
var store = DocumentStore.For(options =>
{
    ...
    options.Projections.Add<BeerCountProjection>(JasperFx.Events.Projections.ProjectionLifecycle.Inline);
});
...
public record BoxAggregate(Guid BoxId, string[] Beers)
{
    ...
    public static BoxAggregate Apply(BeerAdded beerAdded, BoxAggregate previous)
        => previous with
        {
            Beers = previous.Beers.Append(beerAdded.BeerName).ToArray()
        };
}
...
public class BeerCountProjection : EventProjection
{
    public BeerCountProjection()
    {
        ...
        ProjectAsync<BeerAdded>(async (e, operations, token) =>
        {
